...
public class BookRentServiceImplV1 implements BookRentService{
	 ...
	 * execption 처리방법 2가지
	 * 1. try-catch 를 사용하기
	 * 		exception 이 발생할 코드를 감싸서 "직접" exception 처리하기
	 * 
	 * 2. 떠넘기기(던지기)
	 * 		method throw 를 추가하여 호출한 mehtod에게 exception을 되돌려 보내기
	 * 		현재 method 에서 직접 exception을 처리하는 코드가 없어서 코드가 다소 간소화 된다.
	 * 
	 * 		호출한 곳에서 대신 execption을 처리한다.
	 */
	...
	protected List<BookRentDTO> select(PreparedStatement pStr) throws SQLException {
		...
		while(rStr.next()) {
			...
			brDTO.setBr_price(rStr.getString("대여금"));
			brDTO.setBr_edate(rStr.getString("대여일"));
			brList.add(brDTO);
		}
	...
	public BookRentDTO findById(Long seq) {
		 ...
		 * 실제 쿼리는 WHERE 도서명 = 1 OR 1 = 1 과 같이 만들어진다.
		 * 
		...
		try {
			// 아래의 2개 코드에 의해서 WHERE 주문번호 = 값 이 된다.
			pStr = dbConn.prepareStatement(sql);
			pStr.setLong(1, seq);
...
//			rSet.close();
	...
	public int insert(BookRentVO bookRentVO) {
		...
		try {
			...
			// insert. update, delte와 관련된 SQL은 executeUpdate() method 로 처리한다
			// 정상적으로 SQL이 성공하면 result 에는 0 보다 큰 값이 담긴다.
			int result = pStr.executeUpdate();
			// update로 실행시 반드시 close
			pStr.close();
			return result;
