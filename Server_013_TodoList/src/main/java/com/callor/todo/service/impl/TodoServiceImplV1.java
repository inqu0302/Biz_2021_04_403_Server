...
public class TodoServiceImplV1 implements TodoService {
	...
	protected List<Map<String, Object>> select(ResultSet rSet) throws SQLException {
		...
		while(rSet.next()) {
			...
			for(int index = 0 ; index < columns ; index++) {
				...
				// ResultSet으로 부터 index위치의 칼럼에 저장된 실제 데이터 가져오기
	...
	public List<Map<String, Object>> selectAll() {
		...
		// 완료되지 않은(td_edate값이 없는) 데이터를 우선적으로
		// 그후 INSERT된 순서대로 보여주기
...
//			System.out.println("insert 실패");
	...
	private void viewSelect(ResultSet rSet) throws SQLException {
		/*
		 * ResultSet을 DB로 수신받은 데이터에서 메타데이터를 활용하여 List<VO>에 값 세팅하기
		 */
		// rSet으로부터 메타데이터 추출(getter)
		ResultSetMetaData md = rSet.getMetaData();

		// Select된 table의 칼럼정보를 추출
		// 1. 칼럼이 몇개인가
		int columns = md.getColumnCount();
		...
		/*
		 * rSet에는 select table의 결과가 모두 저장되어있다. 실제 데이터와 메타데이터가 모두 포함되어 있다. rSet 객체의
		 * next() method를 한번 호출하면 결과 table의 첫번째 레코드를 읽을 수 있도록 준비 이어서 next() method를 계속해서
		 * 호출하면 한줄씩 순서대로 읽을 수 있도록 준비 만약 더이상 읽을 데이터가 없으면 false를 return한다 while() 반복문을
