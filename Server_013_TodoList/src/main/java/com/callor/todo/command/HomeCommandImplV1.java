...
public class HomeCommandImplV1 implements TodoCommand{
	...
	/*
	 * 서비스등의 클래스를 객체로 생성하여 클래스 내부의 여러 method에서 공용으로
	 * 사용할 경우 객체를 클래스의 맴버영역에 선언한다
	 * 이때 객체의 접근제한자를 protected로 하면 이 클래스를 상속받는 곳에서도 
	 * 고용으로 사용할 수 있다. 이러한 코드가 편하지만 간혹 메모리leak(누수)가 발생할 수 있다.
