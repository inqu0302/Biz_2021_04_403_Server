...
<style>
	...
	*{
	...
	table.td_list{
		...
		/* table 을 그릴때 각 td 와 td 사이의 간격이 있어 선이 끊어지는걸 방지 */
		border-spacing:0;
	}
	...
	table.td_list td.doit{
		...
		/*
		width : 무조건 폭을 지정
		max-width : 포함된 text(content)길이에 따라 조절을 하되 최소 크기를 제한
		*/
		max-width:80%;
		/*
		...
		td가 3개인데
		첫번째, 세번째 td를 20%로 설정했기 때문에 두 td 는 
		전체 talbe width의 60%가 될것이고 이 상태에서 max-width를 0으로 지정하고 
		ellipsis  설정을 해주면 된다.
		*/
		max-width: 0;
		overflow: hidden;
