...
<style>
	/*
		...
		padding을 주었을때 좌,우의 padding 을 밖으로 보내지 않고 내부로
		흡수하는 옵션
		
		이 설정을 style의 맨 상단에 설정하면 padding 으로 인한
		Layout의 흐트러짐을 막을 수 있다.
	*/
	*{
	...
	/*
		table 의 어떤 row에 마우스가 올라가면 바탕색을 rgba(50,150,200, 0.8)로
		마우스 커서 모양을 손모양으로 바꿔준다 
	*/
	...
	a{
		...
		/* 
			style의 상속
			나(a tag)를 감싸고 있는(td) tag의 color가 변경이 되면 같이 바뀐다. 
		*/
		color: inherit;
	}
</style>
...
<body>
	<h1>무엇을 먹을까</h1>
	<%-- form의 method를 POST로 지정ㅇ하면
	COntroller의 doPost() 합수가 받아서 처리를 한다 
			...
			<c:when test="${not empty FOODS}">
				<table>
					...
					<%--
						...
						for 반복문으로 반복하면서 item을 getter하여 FOOD(DTO)에 담아라
					 --%>
					<c:forEach items="${FOODS }" var="FOOD">
					<tr>
					<%-- forEach가 만든 FOOD(DTO)에서 각 변수값을 
					getter하여 화면에 띄워라 --%>
						<td>${FOOD.fd_code}</td>
						<td>
